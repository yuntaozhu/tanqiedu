...
async function classify() {
  ...
  for (const level of prompts) {
    ...
    const parts = [];
    parts.push({ text: `Below are several toy block figures. I need you to identify which is which from this list of names: ${level.projects.join(', ')}. For each image, reply with exactly the mapping in the format: imageXX: <chinese name>` });
    ...
    parts.push({ text: "Now output the mapping for all provided images. Limit output to just lines like 'image62: 小乌龟'." });
    ...
    try {
        ...
        finalOutput += `\n--- ${level.name} ---\n` + response.text + '\n';
        fs.writeFileSync('output-batch.txt', finalOutput);
    } catch(e) {
        console.log(`Failed for level ${level.name}: ${e.message}`);
    }
  }
}
classify();
